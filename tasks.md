...
- [ ] **Feed the brake "real" needle from an actual pressure sensor** — The Race-skin pedals dial (`index.html` `rcDrawPedals`) shows the brake with two needles: amber = commanded target (`orin_cmd_brake`), red = real, mapped to the ESP32 braking feedback channel (`esp32_braking`, `ESP_ACT_BRAKING 0x03`). For the red needle to mean *measured brake pressure* (not just applied effort echoed back), the **kart-medulla firmware must populate `ESP_ACT_BRAKING` from the brake-pressure sensor**. Throttle intentionally has a single needle — no independent throttle sensor exists, the value is the commanded electronic effort. No dashboard change needed once firmware sends real pressure on that channel; if a separate topic is preferred, add a `esp32_brake_pressure` field to `protocol.py` + `dashboard_node.py` and point the red needle at it.
