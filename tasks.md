...
- [ ] **Wire the pneumatic pressure sensors (tank + piston) end-to-end** — The Race-skin System page has a Pneumatics panel (`index.html`): a TANK dial (`RC_DIAL_TANK`) with a green safe band and a PISTON bar. They read `d.pneu_tank_bar` / `d.pneu_piston_bar`, which **no firmware sends yet** — the UI shows `-- bar` and only the demo (`?demo=1`) fills them. Work: (1) **kart-medulla**: add tank + brake-piston pressure sensors and transmit them (new `ESP_*` frame ids, or extend an existing health/braking frame); (2) **kart-brain**: add decode in `protocol.py`, subscriptions in `dashboard_node.py`, and `pneu_tank_bar` / `pneu_piston_bar` to `DashboardState`. (3) **Confirm the real tank safe-band limits** — the dashboard currently uses PLACEHOLDER `PNEU_TANK_MIN=6, PNEU_TANK_MAX=10, PNEU_MAX_BAR=12` (bar); replace with the team's actual EBS reservoir spec (out-of-band makes the System tab pulse red). Note: the **piston pressure is the brake-pressure sensor**, so it can also feed the pedals dial's red "real" brake needle (`rcDrawPedals`, currently mapped to `esp32_braking`) — unify them: the same sensor drives both the piston bar here and the brake-real needle on the TELE page. Throttle stays a single needle (no independent throttle sensor; it is the commanded electronic effort).
