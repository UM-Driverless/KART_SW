...
**Which Wi-Fi network gets joined is `connection.autoconnect-priority`, not route metric.** Only one Wi-Fi connection can be active at a time, so priority picks it; metric only ranks connections that are *already* active, which is why metric is the right lever for the two USB tethers and the wrong one here.

**The phone hotspots deliberately outrank `Robots_urjc`** (100 and 50 against 10), and this should not be "fixed" to prefer the free lab network. Rubén's reasoning, 2026-08-10: the lab Wi-Fi is tied to a place and is lost the moment the kart moves, while the phone travels with the kart; and switching a hotspot on is a deliberate act, so it is a reliable signal that someone wants the kart connected. Ranking by data cost optimises for the bench and fails everywhere else. A cold boot at the workshop therefore joins the phone hotspot rather than the lab network when both are in range — that is intended, not a bug.

**Three consequences of the shared address, measured 2026-08-10:**
- **Connections do not survive a failover.** The Orin's source address is identical on both tethers, but the NAT state lives on the *phone*, so the standby phone knows nothing about sessions the other was translating. A real failover therefore rebuilds the Cloudflare tunnel rather than migrating it (~20 s). Nothing short of a multipath setup changes this, and it is not worth it here.
- **Only one tether carries traffic at a time.** No load balancing, no splitting; the lower metric takes everything and the other is a warm spare.
- **The report-only probe depends on `rp_filter = 2`** (loose), which is what all interfaces currently have. Under strict mode (`1`) a probe bound to the *standby* tether would go out fine and its reply would arrive on that NIC, but the kernel's reverse-path check would look up the route back to `172.20.10.2`, find it points at the winning tether instead, and drop the reply — reporting a perfectly healthy phone as having no internet. Nobody set this sysctl deliberately, so if tether probes ever start logging false failures, check `sysctl net.ipv4.conf.all.rp_filter` first.
