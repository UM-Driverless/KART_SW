...
## 2026-07-31 — Dropped the Orin's only remote route to test it, and the safety net never armed
**What happened:** To test whether the Cloudflare tunnel re-establishes over the Wi-Fi fallback, the USB tether was disconnected in software (`nmcli connection down "Wired connection 2"`). A 4-minute self-restore was supposed to be armed first, in the same SSH command, because dropping the tether kills the session that would otherwise restore it. The restore never fired: 25+ minutes later `kart.rubenayla.xyz` still returned 530 and `ssh orin-remote` was still refused, with nobody near the kart. The test also produced no answer, so the lockout bought nothing.
**Root cause:**
1. *The safety net was never verified to exist before the dangerous action was taken.* The arm command was `sudo bash -c "setsid nohup bash -c \"sleep 240; ...\" ... &"` — three levels of nested quoting through `sudo`, `bash -c` and `nohup`, sent over SSH inside another layer of shell quoting. It printed a confirmation (`pgrep -fc "sleep 240"`), but that output was never read, because the very next command in the same invocation dropped the tether and killed the session before anything flushed. The confirmation was designed and then thrown away.
2. *`nmcli connection down` is a one-way door on the only route in.* NetworkManager marks a manually deactivated connection ineligible for autoconnect, and nothing on the Orin brought the tether back. `wifi-watchdog` manages only the Wi-Fi radio, so no component owned recovery of the wired path.
**Prevention:**
- **Arm and verify a recovery path as a separate, completed step before the step that can cut you off.** Two invocations, never one: arm, read the confirmation, then act. A safety net whose confirmation you cannot read is not a safety net.
- **Prefer `at`/`systemd-run --on-active` over hand-rolled `setsid nohup` chains for delayed recovery** — one level of quoting, and the scheduled job is inspectable (`atq`, `systemctl list-timers`) instead of being a pid you have to guess at.
- **`wifi-watchdog.sh` now owns this**: if connectivity is absent for `INTERNET_GRACE_SECONDS` (120 s) while tether hardware is plugged in, it runs `nmcli device connect` on it. The grace period is what keeps it from fighting a legitimate fallback test, since a successful test has working internet over Wi-Fi. This would have self-healed the incident in two minutes.
- **A remote test that can sever remote access needs its recovery owned by the machine, not by the tester's shell.** The tester's shell is the thing being severed.
